...
select_dashboard_preset() {
    ...
    if [ -n "${LINCE_DASHBOARD_PRESET:-}" ]; then
        case "$LINCE_DASHBOARD_PRESET" in
            minimal|statusline|classic) return 0 ;;
            *) echo "Invalid LINCE_DASHBOARD_PRESET: $LINCE_DASHBOARD_PRESET" >&2; return 1 ;;
        esac
    fi
    echo ""
    echo "Choose your dashboard preset:"
    echo "  1) minimal (default) — compact sidebar and LINCE status bar; more space for agents."
    echo "  2) statusline — LINCE status bar only; sidebar initially hidden (Alt+s to show it)."
    ...
    while true; do
        ...
        case "$choice" in
            ...
            2|statusline) LINCE_DASHBOARD_PRESET=statusline; break ;;
