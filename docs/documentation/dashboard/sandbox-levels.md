...
|-----------------------------------------|-------------------------------|------------------------|-----------------------|--------------------|---------------------------------------------------------|
| Network (allowed destinations)          | Anthropic API only            | Anthropic API only     | Anthropic API via proxy allowlist, ⚠ plus ANY host service on 127.0.0.1 | inherited base     | + GitHub + gh CLI domains                               |
| Network isolation enforcement           | kernel (netns) + proxy allowlist | kernel (Landlock + nono policy) | Seatbelt `(deny network*)` loopback-only + proxy allowlist (shared host loopback — no netns) | inherited       | proxy allowlist                                         |
| Agent opens raw socket to exfiltrate    | blocked (no route in netns) ✓ | blocked (kernel policy) ✓ | blocked for non-loopback ✓ — ⚠ any 127.0.0.1 listener (incl. a local open proxy) is still reachable | depends on base | blocked for non-allowlisted hosts (proxy)               |
...
| Credential proxy                        | host-side, kernel-gated       | host-side, kernel-gated| host-side, Seatbelt-gated (loopback-only) | opt-in             | off by default                                          |
...
| GPU (`[sandbox].expose_gpu`, #280)      | **hidden** — global flag ignored; fragment must opt in | host `/dev` visible (Landlock doesn't virtualize /dev) | n/a (no CUDA on macOS) | hidden by default; global opt-in | hidden by default; global opt-in |

### 2.1 Paranoid

**Network.** Only the Anthropic API is reachable over the internet, and isolation is enforced below the agent on all backends (kernel namespaces on bwrap, Landlock + nono policy on nono, the Seatbelt MAC layer on macOS — with a seatbelt-specific localhost caveat, see below).
...
- Under seatbelt: the generated paranoid profile denies all non-loopback network (`(deny network*)` plus loopback-only allows), so direct egress is blocked and API traffic goes through the host-side credential proxy on `127.0.0.1`, which enforces the same application-layer allowlist (any CONNECT or HTTP request outside `allow_domains` ∪ credential-rule domains is rejected with 403). **Residual risk — shared host loopback**: Seatbelt has no network namespace, so `127.0.0.1` inside the sandbox is the *host's* loopback, and the agent can reach **any** service listening on it — local databases, dev servers, Ollama, Docker's TCP API, and notably **other concurrently running lince agents' credential proxies** (each proxy accepts any local client and injects its credentials). A localhost open proxy or `ssh -D` SOCKS would also re-open egress. bwrap paranoid does not have this exposure: its netns gives the sandbox a *private* loopback. The remaining gaps vs bwrap paranoid are this shared loopback and the missing PID namespace. Containment is keyed on `[security] unshare_net = true` (set by the shipped paranoid fragments and inherited by levels that `extends` them), the same flag that arms the proxy allowlist.

The credential proxy injects the API key on the host side in all cases, so the key never enters the sandbox environment. Everything else — DNS for arbitrary hosts, plain HTTPS to other services, even `pip install` from PyPI — fails.
...
- **paranoid** when you are running an agent on an untrusted prompt, an unfamiliar repository, or any input you wouldn't paste into a root shell. The agent still does its job for the LLM API path; everything else is denied. Paranoid requires a **credential on the host** (e.g. `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY` — and for Claude Code, `CLAUDE_CODE_OAUTH_TOKEN` is also recognized) so the credential proxy has something to inject — OAuth/browser-login flows that store the token only inside the agent's config dir put it inside the sandbox, breaking paranoid's "credentials never in the sandbox" guarantee. Claude Code OAuth users can `export CLAUDE_CODE_OAUTH_TOKEN` (e.g. from `claude setup-token`) so the proxy injects it host-side as `Authorization: Bearer`. Otherwise, if your agent is OAuth-authenticated, pick `normal` or `permissive` instead, or switch the agent to API-key auth. The trust-model rationale and an opt-out recipe (with risks spelled out) live in §6, "Trust model: why paranoid requires API-key auth (and how to opt out)". See §8 for per-agent specifics.
...
- `sandbox_backend = "seatbelt"` — Apple Seatbelt (`sandbox-exec`), macOS only, filesystem isolation via Seatbelt's mandatory access control. At paranoid the generated profile is loopback-only (`(deny network*)` + loopback allows) so direct egress goes through the allowlist-enforcing credential proxy — but the loopback is the *host's* shared loopback (no network namespace), so any service listening on `127.0.0.1` remains reachable. Does **not** provide PID namespace isolation either; warnings for both are printed at startup.
...
**Backend implementation note.** All three backends contain network under paranoid; the implementation paths differ. bwrap paranoid uses `--unshare-net` plus a socat unix-socket bridge to the credential proxy. nono paranoid uses Landlock LSM plus nono's native network policy. Seatbelt paranoid uses a loopback-only profile (`(deny network*)` + loopback allows, enforced by the Seatbelt MAC layer): direct internet egress is blocked and API traffic goes through the host-side credential proxy on `127.0.0.1`, which enforces the domain allowlist. From a threat-model standpoint, seatbelt paranoid is weaker than bwrap/nono paranoid in two ways: PID isolation (host processes stay visible) and the **shared host loopback** — Seatbelt has no network namespace, so the agent can reach any service listening on `127.0.0.1` (other agents' credential proxies, local databases, dev servers, Docker's TCP API), whereas bwrap's netns gives the sandbox a private loopback. Warnings are printed at startup listing exactly what is unavailable.
...
That last row is the entire reason paranoid exists. If you opt into the bypass, you keep the network and filesystem isolation but lose the credential isolation. The shipped paranoid fragments therefore only auto-map host-side credentials (API keys, plus `CLAUDE_CODE_OAUTH_TOKEN` for claude) and bail out when none is set — picking `normal` or `permissive` for OAuth users is the **safer** path; those levels don't claim "no credentials in the sandbox" so OAuth fits their model honestly.

**Claude Code exception**: Claude Code can export its OAuth credential as a host env var (`claude setup-token` → `export CLAUDE_CODE_OAUTH_TOKEN=...`). The credential proxy recognizes `CLAUDE_CODE_OAUTH_TOKEN` and injects it host-side as `Authorization: Bearer`, exactly like an API key — so claude OAuth users get the proper paranoid flow without the opt-out recipe below. On conflict with `ANTHROPIC_API_KEY`, the Bearer credential wins (a warning is printed); if `ANTHROPIC_AUTH_TOKEN` is also set, it wins over `CLAUDE_CODE_OAUTH_TOKEN`.

#### How to opt out (if you really want OAuth + paranoid)

This recipe is generic — it works for any agent (gemini, claude, codex, opencode, pi). You're trading the credential-isolation guarantee for kernel network isolation while keeping OAuth. (Claude Code users usually don't need this: `export CLAUDE_CODE_OAUTH_TOKEN` is recognized by the credential proxy and gives you the proper paranoid flow — see the Claude Code exception above.)
