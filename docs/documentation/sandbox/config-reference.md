...
| `expose_gpu` | bool | `false` | Expose the host GPU inside the bwrap sandbox (#280). The default `/dev` is a minimal synthetic devtmpfs, so CUDA/GPU runtimes silently fall back to CPU; when true, every `/dev/nvidia*` node plus `/dev/dri` is re-exposed via `--dev-bind-try` (multi-GPU friendly, no-op on GPU-less hosts). Off by default — the GPU driver ioctl surface is extra kernel attack surface. **Ignored at `paranoid`**: the global flag never reaches paranoid runs; to expose the GPU there, set `expose_gpu = true` inside the paranoid fragment itself. The spawn banner shows `gpu exposed` when active |
