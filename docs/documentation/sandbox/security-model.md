...
| **Linux only (bwrap backend)** | The bubblewrap backend depends on Linux kernel namespaces. macOS users can use the native Seatbelt backend (`sandbox-exec`, built into macOS). The legacy [nono](https://github.com/always-further/nono) backend is deprecated — see [Migrating from nono to Seatbelt](migration-nono-to-seatbelt.md) |
...
> **macOS validation status.** The Seatbelt profile generation and the credential-proxy allowlist are covered by host-side tests (`sandbox/tests/`; they pass on Linux and macOS). The behavioral claims in this section were **validated on real macOS hardware on 2026-06-10** (macOS 26.5.1, Apple Silicon, `sandbox-exec` accepting the loopback SBPL forms at runtime): loopback-only containment (direct egress kernel-denied), proxy-allowlist egress (403 on non-allowlisted CONNECT), DNS failing for arbitrary hosts under `(deny network*)`, the fail-closed paranoid guard, and the shared-loopback residual — full matrix in [#196](https://github.com/RisorseArtificiali/lince/issues/196#issuecomment-4669462339). One known divergence from bwrap to keep in mind: SBPL `(deny network*)` also denies **unix-domain sockets** (Seatbelt models them as network operations, and the loopback allows only cover TCP/IP), whereas bwrap's network namespace leaves unix sockets working — that is how the Linux paranoid proxy bridge over `/run/lince-proxy.sock` operates. On macOS at paranoid, loopback TCP is the only in-sandbox transport.
