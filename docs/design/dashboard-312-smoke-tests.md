...
Senza preset esplicito viene usato `minimal`, anche aggiornando configurazioni
precedenti. Per la vista classica specificare `--preset classic`.
...
- `S` e `-` restano fissi sulla seconda riga. Nei passaggi `R → I/P → R`,
  pallino e lettera devono aggiornarsi senza lasciare caratteri residui.
...
- Ciclare `Alt+b` quattro volte: assente → solo riepilogo sinistro → completa → solo agenti. Alternare anche `Alt+s`. Con entrambe nascoste,
...
- In modalità riepilogo verificare che restino numeri/stati e animazioni a sinistra,
  senza nomi a destra. Le tre modalità visibili occupano due righe.
...
- In modalità solo destra non deve comparire il conteggio: i nomi usano tutta la larghezza.
  Salvare con `Alt+q` e rilanciare: anche questa modalità deve essere ripristinata.

- Aprire `Alt+d`: nella legenda in basso `[n]` e `[x]` devono essere leggibili,
